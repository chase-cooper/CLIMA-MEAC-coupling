...
c     COMMON/CBLOK/FO2,FN2,FCO2,FAR,FCH4
     ...
     & ,DLSCDT,CCL,CCS 
      ...
      common/relhum_eddy/relhum_eddy_july_new
...
c            2x-CO2 condensation   
      !print*,'CALLING CONVEC.F'
      !print*,'TTAB convec.f',TTAB
      itrop = itrop !EWS - no complaining 
      NDIV = 10     !Why is this hardcoded? JDW2021  !number of sublevels
      PLT = DZ
...
c      print *, 'FH1=', FH1
      ...
      imco2=0.      !imco2 flag, set to zero at first. JDW 2021
...
c      if(FCSAT.lt.FC1/1.34) imco2=1  ! supersaturating to 1.34?
      ...
      if(imco2.eq.0) go to 3 !This means that where CO2 is not saturated, convection happens with water JDW 2021
...
c        DLPDLT = CPN/R
cTDR    using CPNW, which includes CO2 and non-condensibles (i.e., all but H2O)
        DLPDLT = CPNW/R !!!Important JDW for ensuring EddySed is consistent
c        print *, 'CPNW=',CPNW, 'R=', R, 'DLTDLP', 1/DLPDLT
        cflag = 2.
        GO TO 2
      END IF
...
! --------------------------------------------------------------------
  ...
  14  CONTINUE
      ...
      !TL=TL+DLP/DLPDLT
      ! print*,'EXP(DLPDLT/DLP +TL)',EXP(TL+DLP/DLPDLT),P1,L
...
! so we can just use the ideal gas law equation for DLPVLT
...
!-----------------------------------------------------
      ...
      DLPDLT = 18.*HLT/R - DADLT/AV/(1. + AV*AMN/18.) !DlogT/DlogP for moist adiabat
      !print*, DLPDLT,DLPDLT,HLT,R,DADLT,AV,T
...
C Dry adiabat
  40  DLPDLT = CPNW*AMN/(4.184*R)
   ...
   2  DLT = DLP/DLPDLT     
      ...
      TL = TL + DLT
      T = EXP(TL) !Temperature from the lapse rate.
   ...
   1  CONTINUE       !End Sublevel integration
      ...
      T2 = T !Later, in ClimaMain, if T2 is greater than the T, it is convective. 
      ...
      !print *,'FH2',FH2,PSAT,P2,RELHUM(P2)
      !print*,'relhum(p2)',RELHUM(P2),P2

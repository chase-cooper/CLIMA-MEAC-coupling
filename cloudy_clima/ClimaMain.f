...
c     vectors for gaussian zenith angles      
     ...
     & ,FI_cloudy(NS1,ND)   ! Added FH2 to CBLOK 5/29/2012 c-rr
      DIMENSION AO2(ND),AN2(ND),AH2(ND),BASEH2O(ND) ! Profiles for species not in FI --CC 2025
...
c
     ...
     &              status='old')     ! CO2 ir coefficients
      ...
      OPEN(unit=10, file=DIRDATA//'/STELLAR_SPECTRA_update.pdat',status='old') !EWS - new stellar spectra updated
...
c       Temperature profiles
      OPEN(unit=700,file=DIRINOUT//'/extras/surftemp.dat')
      OPEN(unit=701,file=DIRINOUT//'/extras/toatemp.dat')
c       Water-related profiles
      OPEN(unit=702,file=DIRINOUT//'/extras/relhumvec.dat')
      OPEN(unit=703,file=DIRINOUT//'/extras/fsatur.dat')
      OPEN(unit=704,file=DIRINOUT//'/extras/relhum.dat')
      OPEN(unit=705,file=DIRINOUT//'/extras/constants.dat')
...
51    FORMAT(4/)
      ...
      READ(1,*) AA,PG0          !Surface pressure (bar)
...
c Reading mixing ratio profiles     CC2025
      DO K=1, ND
            READ(500,*) AO2(K)                 ! Oxygen
            READ(501,*) AN2(K)                 ! Nitrogen
            READ(502,*) AH2(K)                 ! Hydrogen
            READ(503,*) FI(2,K)                ! CO2
            READ(504,*) FI(3,K)                ! CH4
...
c   Not needed, we import our own ozone profile cc2025
...
c Building the water profile
      if(ICOUPLE.eq.0)then
c Not needed, we import our own profile
c        DO J = 1,ND
c         FI(1,J)=FSAVE(J)!eddyqt(J,1)-eddyqc(J,1)!FSAVE(J)
c        END DO
      !   do J=1,ND
...
c jkf 6/26/08 Change H2O initialization in the stratosphere
      ...
      ENDDO
c Not needed, already done
c      FI(5,J) = max(FC2H6,1.e-60) !ethane !EWS - debug for low mixing ratios
c   2  FI(3,J) = max(FCH4,1.e-60)  !methane !EWS - debug for low mixing ratios
c
c jfk 6/27/08
      do j=1,nd
...
C-TF  FOR NOW IO3 IS SET TO 0
       ...
       if(IO3.eq.1.and.ICOUPLE.eq.0) then
c   Commenting this out, using imported ozone mixing ratios only    cc2025
c            CALL OZONE(FI,P)
          !do i=1,ND
           !  read(22,*) x, FI(4,i) 
          !enddo
...
C************************************************************
      ...
      WRITE(700,*) T(ND)
      WRITE(701,*) tclim(1)
      CALL ALTITUDE(NST,T,FI,DZ)    ! testing cc2025
      ITROP = 1
c      PRINT 161,NST
c 160  FORMAT(/1X,"---------------------------------------------",
c     2  //1X,"NST =",I6)
...
!       Attempting to clamp uppermost value of tclim, might be causing crashes  cc2025

      IF(T(1).gt.tclim(1)) tclim(1)=MIN(tclim(1)*1.1,T(1))
      IF(T(1).lt.tclim(1)) tclim(1)=MAX(tclim(1)*0.9,T(1))
...
c   Updated to reference custom gas arrays cc2025
      mw_atmos=FI(2,nd)*44.01 + (1.0-FI(2,nd))*(FI(3,nd)*16.04 +
     & FI(4,nd)*48 +  AO2(nd)*32.0 + FNO2*46.0055 + FI(5,nd)*30.06904
     & + AH2(nd)*2.01588 + FAR*39.948 +AN2(nd)*28.0134) ! Do i need to instead caluclate the moist molecular weight? JDW 2023
...
C
       ...
       CPNC = AN2(J)*CPN2(J) + AO2(J)*CPO2(J) + FAR*4.97 +FI(3,J)*8.3
...
C     ==================================================================================
      ...
      !print*,'Calling Solar' !Each Solar code is called six times for the weightt* array
        ...
        !print *, '1608', fdnsol_cloudy(1)
C     =====================================================================
          do j=1,nd !JDW
          !print*,j,'fdnsoltot pre',fdnsol_clear(j)
...
C !2022  FORMAT(I3,2(1x,1PE10.4),8(1X,1PE11.4))
...
c          FI(2,J1-1)=FC2
c          FI(2,J1-1) = FCO2  ! jiggered again
        ELSE
            ITROP=0
            FLAGCONVEC(J1)=0.
        ENDIF
...
c Water from the cold trap to the ground
            if(doEddy)then
                  ...
                  DO J = 1, ND      ! Changed upper layer from JCOLD to 1 cc2025
...
C         IF (FLAGCONVEC(J).LT.1.E-3) THEN
          ...
          tclim(J)=TN(J)    ! This undoes previous line? cc2025
...
C   debug, resetting water profile after each step  cc2025
      IF (FIXH20.eq.1) THEN
            DO B=1,ND
                FI(1,B) = BASEH2O(B)
            ENDDO
...
!      & FI(5,zz)-FAR)*FN2 !N2
     ...
     & - AO2(zz) - FNO2 -FI(5,zz)-AH2(zz)-FAR  !Nitrogen 7
...
c       Write final mixing ratio profiles
      DO J=1,ND
            ...
            WRITE(601,*) AN2(J)
            WRITE(602,*) AH2(J)
            WRITE(603,*) FI(2,J)    ! CO2
            WRITE(604,*) FI(3,J)    ! CH4
            WRITE(605,*) FI(5,J)    ! C2H6
            WRITE(606,*) FI(1,J)    ! H2O
            WRITE(607,*) FI(4,J)    ! O3
      ENDDO

c       Write water vapor-adjacent quantities
      DO J=1,ND
            WRITE(702,*) relhum_vec(J)
            WRITE(703,*) fsatur(J)
            WRITE(704,*) RELHUM(P(J))
      ENDDO
      WRITE(705,*) 'PSAT = ',PSAT
      WRITE(705,*) 'FH2 = ',FH2
      WRITE(705,*) 'upatm_mix = ',upatm_mix

...
void SimChemCarbon(double result[], double T);

void SimChemCarbon(double result[], double T)
{
     /* Only valid for hydrogen rich */
        
       int j, dum;
       char atom[NATOMS][3];
       double k, b[NATOMS], PH, AC, AO;
       double a1=1.106131E+6;
       double b1=-5.6895E+4;
       double c1=62.565;
       double d1=-5.81396E-4;
       double e1=2.346515E-8;
       FILE *file;
       
       k=exp((a1/T+b1+c1*T+d1*pow(T,2)+e1*pow(T,3))/(R_GAS*0.239)/T);
       
       file=fopen(ATOM_ABUN_FILE, "r");
       for(j=0; j<NATOMS; j++){
       ...
       AC=b[2]/b[0];
       AO=b[4]/b[0];
       result[1]=AC+AO+PH*PH/2/k-sqrt(pow(AC+AO+PH*PH/2/k,2)-4*AC*AO); /* CO  */
       result[2]=2*AC-result[1]; /* CH4 */
       result[3]=2*AO-result[2]; /* H2O */

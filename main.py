...
def updateMEAC():
    ...
    ### Plotting surface temperature over time 
    fig,axes = plt.subplots()

    # plotting temperatures
    n = np.linspace(1,len(surfTemps),len(surfTemps))
    axes.plot(n,surfTemps,color='limegreen')

    # Plotting where coupling occurs
    CLIMAstepIntervals.append(len(surfTemps)+0.5)
    axes.vlines(CLIMAstepIntervals,ymin=np.min(surfTemps)-1,\
    ...
    ### Write temperature-pressure profile. Interpolate over range CLIMA misses
...
for i in range(NLOOPS):
    updateCLIMA(firstloop=(i==0 and not RESUMERUN))
    runCLIMA(i)
    updateMEAC()
    runMEAC(i)
    os.system('clear')

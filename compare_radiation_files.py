...
def plot_comparison_rad(base_file, base_numrad, base_numz, new_file, new_numrad, new_numz, name):
    ...
    return new_wavs,new_rad[-1,:]           # -1 for TOA, 0 for BOA

###############################
###Run
###############################

# x,y = plot_comparison_rad('hu-code-sr/scenario_library/CO2_CH4/fco2_1e-1/Radiation.dat', 1000, 50,'hu-code-sr/scenario_library/CO2_CH4/fco2_1e-1/Radiation.dat', 1000, 50, 'testrad')

# meac_wvlns = x      # nm
# meac_fluxes = y*1e-7*1e4*1e3     # originally erg/cm^2/s/um? converted to W/m^2/nm?? They're the same???

# g = np.loadtxt('hu-code-sr/Data/solar0.txt')
# solw,solf = g[:,0],g[:,1]

# fig,ax = plt.subplots()
# ax.plot(meac_wvlns,meac_fluxes,c='red',label="TOA Flux")
# ax.plot(solw,solf,c='blue',label='Solar spectrum')
# plt.xscale('log')
# plt.yscale('log')
# plt.ylim(1e-10,1)
# plt.legend()

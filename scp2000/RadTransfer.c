	...
	for (i=0; i<NWave; i++) {
		...
		if (IFDIFFUSE == 1) {
			...
			for (j=1; j <= zbin; j++) {
				...
				j1    = zbin+1-j;
				for (k=1; k<=nump; k++) { 
					wa[j] += xx[j1][stdn[k]]*cross[k][i]/qysum[k];
				}
			...
			/*SR addition: NTau should always be at least 1, otherwise two stream formalism does not make sense.*/
			if (ntau < 1){
				ntau=1;
			}
			...
			for (j=1; j < ntau; j++) {
				...
				E[l] = e3[j]*(CP0[j+1]-CP1[j]) + e1[j]*(CM1[j]-CM0[j+1]);
			}
			for (j=1; j < ntau; j++) {
				l = 2*j;
...
// 				printf("%s\n", "Radiation");
// 				for (j=0; j<=zbin; j++) {
// 					printf("%le %le \n", radiation[i][j], dflux[zbin-j]);
// 				}
// 				printf("%s\n", "Aflux[0]");
// 				printf("%le %le %le %le %le %le \n", aflux[0], Y1[1], e3[1], Y2[1], e4[1], CP0[1]);
// 				/*printf("%le %le %le %le %le %le %le %le %le %le %le %le %le\n\n", g1[1], w1[1], gamma1[1], gamma2[1], gamma3[1], gamma4[1], lambda[1], gamma[1], tau1[1], e1[1], e2[1], e3[1], e4[1]);*/
// 				
// 				printf("%s\n", "g, w in loop");
// 				for (j=1; j<=ntau; j++) {
// 					printf("%le %le %le %le %le %le\n", gc[j-1], gc[j], g1[j], wc[j-1], wc[j], w1[j]);
// 				}
// 				
// 				printf("%s\n", "g, w manually");
// 				printf("%le %le %le %le %le %le\n", gc[0], gc[1], g1[0], wc[0], wc[1], w1[0]);
// 				printf("%le %le %le %le %le %le\n", gc[1], gc[2], g1[1], wc[1], wc[2], w1[1]);
// 				
// 				/*for (j=0; j<=ntau; j++) {
// 					printf("%i %le %le %le %le %le %le %le %le %le %le\n", j, tc1[j], aflux[j], Y1[j], Y2[j], e1[j], e3[j], e2[j], e4[j], CP1[j], CM1[j]);
// 				}
// 				printf("%s\n", "Interp2");
// 				for (j=0; j<=zbin; j++) {
// 					printf("%i %le %le \n", j, tc[j], dflux[j]);

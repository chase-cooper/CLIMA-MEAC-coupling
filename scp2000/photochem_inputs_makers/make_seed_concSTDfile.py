...
def MEAC_interp(x_new, x_old, y_old):
    ...
    for ind in range(0, len(x_new)):
        ...
        if x_new_val==x_old[0]: #if the interpolation value is at the lower limit of x_old
            ...
            j=np.squeeze(np.where(x_old<x_new_val))[-1] #this is the index at which the old data is just under the interpolation value. so for [0, 1, 2, 3] and 0.5, it would return 0. 
            # pdb.set_trace()

...
void ReactionRateT()
{
	...
	double k_inf, k_0_M, k_f, k_f_0_M, k_f_inf, F, F_c, M_M_c,N;
     ...
     for(i=1;i<=zbin;i++)
     {
       ...
       k_0_M=9.4E-9*exp(-33140.0/tl[i])*MM[i]*THREEBODY;
	   k_inf=3.7E13*exp(-36220.0/tl[i]);	   
       kkT[i][14]=k_0_M/(1.0+k_0_M/k_inf);
	   
       kkT[i][15]=2.49E-8*exp(-28745.0/tl[i])*MM[i];
       kkT[i][16]=3.47E-10*exp(-20326.0/tl[i])*MM[i];
       kkT[i][17]=5.0E4;
       ...
       kkT[i][39]=1.66E-9*exp(-64.0/tl[i])*MM[i];
       ...
       k_inf = 3.38E+11*pow(tl[i]/300,-1.0)*exp(-29500.0/tl[i]);      
       kkT[i][42]=k_0_M/(1.0+k_0_M/k_inf);

       k_0_M = 4.16e-7*pow(tl[i]/298.0,-3.29)*exp(-9610.0/tl[i])*MM[i]*THREEBODY;
       k_inf = 2.03e11*pow(tl[i],0.9)*exp(-9240/tl[i]);
       kkT[i][43]=k_0_M/(1.0+k_0_M/k_inf);
     
	   kkT[i][44]=0.00;//ZEROED BC Wrong reaction. This refers to thermal decay of HSOO, not HSO2. Zeroed to prevent accidental use; should be removed entirely eventually. //4.61E-4*pow(tl[i]/298.0, -5.87)*exp(-15635.4/tl[i])*MM[i];
       ...
       double a1[3] = {3.5795335e+00, 2.5000000e+00, 4.2375461e+00};
       double a2[3] = {-6.1035369e-04, 0.0000000e+00, -3.32075257e-03};
       double a3[3] = {1.0168143e-06, 0.0000000e+00, 1.40030264e-05};
       double a4[3] = {9.0700586e-10, 0.0000000e+00, -1.34239995e-08};
       double a5[3] = {-9.0442449e-13, 0.0000000e+00, 4.37416208e-12};
       double a6[3] = {-1.4344086e+05, 2.5473660e+05, 3.87241185e+03};
       double a7[3] = {3.5084093e+00, -4.4668285e-01, 3.30834869e+00};
       ...
       k_0_M = 1.17e-06*pow(tl[i]/298, -3.0)*exp(-12268/tl[i])*MM[i]*THREEBODY;
       k_inf = 6.8e+13*exp(-13230/tl[i]);
       F_c=0.97-tl[i]/1950;
       M_M_c=k_0_M/k_inf;
       ...
       kkT[i][53]=k_0_M / (1.0 + k_0_M / k_inf);
		 ...
		 k_0_M=1.93E-4*pow(tl[i]/298.0,-2.44)*exp(-62782.1/tl[i])*MM[i];
